...
if (!empty($_SESSION['customer_logged_in']) && !empty($_SESSION['customer_id'])) {
    if (intval($_SESSION['customer_id']) === intval($order['CustomerID'])) $allowed = true;
}
if (!$allowed && session_id() === $order['SessionID']) {
    $allowed = true;
}
if (!$allowed) {
    http_response_code(403);
    echo "<div class='container py-5'><h3>Not authorized to view this invoice.</h3></div>";
    exit;
}
...
<div class="wrap">
  ...
  <div class="meta">
    ...
    <div class="box">
      ...
      Customer: <?= $order['CustomerID'] ? intval($order['CustomerID']) : 'Guest' ?><br>
  ...
  <?php if (!$download): ?>
    <div style="margin-top:18px; text-align:right">
      ...
      <a href="my_orders.php" style="margin-left:8px;display:inline-block;padding:8px 14px;border-radius:6px;border:1px solid #0d6efd;color:#0d6efd;text-decoration:none">Back to Orders</a>
    </div>
  <?php endif; ?>
</div>
</body>
