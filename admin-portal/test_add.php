...
if ($_SERVER["REQUEST_METHOD"]=="POST") {
  ...
  $sql = "INSERT INTO tests VALUES('$id','$pid','$name','$code','$roll','$date',$dept,'$result','$remarks',$tester)";
  $conn->query($sql);
...
<div class="container py-5">
  ...
  <form method="POST">
    ...
    <div class="mb-3"><label>Tested By</label>
      <select name="TestedBy" class="form-control">
        <?php $u=$conn->query("SELECT * FROM users"); while($row=$u->fetch_assoc()){echo "<option value='{$row['UserID']}'>{$row['UserName']}</option>";} ?>
      </select>
    </div>
